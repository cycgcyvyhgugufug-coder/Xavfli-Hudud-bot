...
async def gift_random_yes(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("🎁 <b>Hadyalar jo'natilmoqda...</b>\n\nIltimos, kuting.", parse_mode="HTML")
    ...
    await state.clear(); await call.message.answer(
        f"🎁 <b>Hadyalar jo'natildi!</b>\n\n"
        f"📨 Yuborildi: <b>{sent} ta</b>\n"
        f"❌ Yetkazilmadi: <b>{failed} ta</b>",
        parse_mode="HTML",
        reply_markup=kb.gift_back_kb()
    )
...
async def claim_gift_handler(call: types.CallbackQuery):
    ...
    if status=="won":
        ...
        if campaign[4] >= campaign[3] and await db.mark_gift_completed_notified(gift_id):
            ...
            lines=[
                "🎁 <b>Hadyalarni olgan obunachilar</b>",
                "",
                f"💎 <b>Tarif:</b> {campaign[2]}",
                f"👥 <b>Jami:</b> {len(winners)} ta",
                "",
                "🏆 <b>G‘oliblar:</b>",
                ""
            ]
...
async def set_info_start(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "Yangi matinni jo'nating!",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="Orqaga", callback_data="settings_back")]
        ])
    )
