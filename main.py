...
async def about_handler(message: types.Message):
    default_text = ("Ushbu bot orqali siz turli sifatdagi (360p, 480p, 720p, 1080p) videolarni tomosha qilishingiz mumkin!\n\n"
    ...
    text = await db.get_setting("bot_info") or default_text
...
async def gift_by_id_start(call: types.CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID: return
    await call.answer()
    await state.clear(); await call.message.edit_text("Foydalanuvchining ID raqamini jo'nating!", reply_markup=kb.gift_back_kb()); await state.set_state(AdminGifts.waiting_for_user_id)
...
async def gift_by_id_user(message: types.Message, state: FSMContext):
    ...
    await message.answer("VIP obuna tarifini tanlang!", reply_markup=kb.gift_tariffs_kb("gift_tariff", include_back=True))
...
async def gift_id_tariff(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("🎁 Hadya qilganingizni tasdiqlang!", reply_markup=kb.gift_confirm_kb("gift_id_yes","gift_id_no", include_back=True))
...
async def gift_random_start(call: types.CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID: return
    await state.clear(); await call.message.edit_text("🎁 Hadya qilmoqchi bo'lgan VIP tarifini tanlang!", reply_markup=kb.gift_tariffs_kb("random_tariff", include_back=True)); await state.set_state(AdminGifts.waiting_for_random_tariff)
...
async def gift_random_tariff(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.answer()
    days=int(call.data.rsplit("_",1)[1]); name=GIFT_TARIFFS[days]
    await state.update_data(random_days=days, random_tariff=name); await call.message.edit_text("Nechta odam olishi mumkin sonini yozing!", reply_markup=kb.gift_back_kb()); await state.set_state(AdminGifts.waiting_for_random_count)
...
async def gift_random_count(message: types.Message, state: FSMContext):
    ...
    await state.update_data(random_count=count); await message.answer("🎁 Hadyani jo'nataverymi xo'jayin?", reply_markup=kb.gift_confirm_kb("gift_random_yes","gift_random_no", include_back=True)); await state.set_state(AdminGifts.waiting_for_random_confirm)
...
async def gift_revoke_start(call: types.CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID: return
    await state.clear(); await call.message.edit_text("Kimdan VIPni olib qo'ymoqchisiz ID raqamini yuboring!", reply_markup=kb.gift_back_kb()); await state.set_state(AdminGifts.waiting_for_revoke_id)
...
async def gift_revoke_id(message: types.Message, state: FSMContext):
    ...
    await state.update_data(revoke_user_id=uid); await message.answer("Haqiqatdan ham olib qo'ymoqchimisiz?",reply_markup=kb.gift_confirm_kb("gift_revoke_yes","gift_revoke_no", include_back=True)); await state.set_state(AdminGifts.waiting_for_revoke_confirm)
...
async def set_info_start(call: types.CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID:
        return
    await call.answer()
    await call.message.edit_text("Yangi matinni jo'nating!", reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[[types.InlineKeyboardButton(text="Orqaga", callback_data="settings_back")]]))
    await state.set_state(AdminSettings.waiting_for_info)

@dp.message(AdminSettings.waiting_for_info, F.text)
async def set_info_save(message: types.Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID:
        return
    text = message.text.strip()
    if not text:
        await message.answer("Matn bo'sh bo'lmasligi kerak. Yangi matinni jo'nating!")
        return
    await db.set_setting("bot_info", text)
    await state.clear()
    await message.answer(
        "Matin saqlandi ma'lumot almashtirildi.",
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[[types.InlineKeyboardButton(text="Orqaga", callback_data="settings_back")]]
        )
    )
