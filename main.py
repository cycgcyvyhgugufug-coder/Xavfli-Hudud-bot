...
async def video_code_handler(message: types.Message):
    if message.from_user.id == ADMIN_ID and message.text in ["Video qo‘shish", "Video o'chirish", "Statistika", "Sozlamalar"]:
        return # handeled by admin filters

    video = await db.get_video_by_code(message.text)
    if not video:
        return # Or answer not found
...
async def like_handler(call: types.CallbackQuery):
    ...
    prev_like = await db.has_liked(call.fromuser.id, video_id)
