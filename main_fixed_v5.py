...
from states import AdminAddVideo, AdminDeleteVideo, AdminSettings, UserPayment, AdminBroadcast
...
async def count_view_once(user_id, video_id):
    ...
    if key not in counted_views:
        counted_views.add(key)
...
async def _send_broadcast(target, data):
    ...
    for user_id in users:
        try:
            ...
            sent += 1
            # Telegram flood limitlariga xavfsizroq rioya qilish.
            await asyncio.sleep(0.05)

        except Exception as e:
            # Bloklagan/o'chirilgan akkauntlar yoki boshqa Telegram xatolari
            # sabab butun reklama yuborish jarayoni to'xtab qolmasin.
            failed += 1
...
async def broadcast_target_handler(call: types.CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data()
    required = ("photo_id", "ad_text", "button_name", "button_url")
    ...
    await call.message.edit_text(
        f"📢 Reklama {target_name}ga yuborilmoqda...\n"
        "Iltimos, yuborish tugaguncha kuting."
    )
